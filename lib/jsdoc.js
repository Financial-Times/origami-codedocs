...
class JsDoc extends Codedoc {
    ...
    async generateFor(codeDirectory) {
        ...
        // Validate the JS directory includes a path with '.js' extension,
        // or a sub 'js' directory.
        const dirHasJs = (dir) => {
            const paths = dir ? fs.readdirSync(dir) : [];
            return paths.some(p => p.match(/js$/));
        };
        ...
        let jsdocOutput = '';
        try {
            const { stdout } = await execa.shell(`${require.resolve('jsdoc/jsdoc')} --explain --recurse --nocolor ${mainJs} ${srcJs} ${libJs} | sed  's|'${codeDirectory}'||g'`);
            jsdocOutput = stdout;
        } catch (error) {
